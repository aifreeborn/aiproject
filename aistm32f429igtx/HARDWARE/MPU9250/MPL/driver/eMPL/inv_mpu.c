...
#define i2c_write   ai_mpu9250_write
#define i2c_read    ai_mpu9250_read 
#define delay_ms    ai_delay_ms
...
// 陀螺仪方向设置
static signed char ai_gyro_orien[9] = {
    1, 0, 0,
    0, 1, 0,
    0, 0, 1
};
...
// 磁力计方向设置
static signed char ai_comp_orien[9] = {
    0, 1, 0,
    1, 0, 0,
    0, 0, -1
};
...
// 方向转换
unsigned short ai_inv_row2scale(const signed char *row)
{
...
/*
********************************************************************************
*    Function: ai_mpu_dmp_init
* Description: mpu9250,mpl初始化
*       Input: void
*      Output: None
...
int ai_mpu_dmp_init(void)
{
    ...
    if (mpu_init(&int_param) == 0) {
        ...
        if (ret)
            return -3;
        ret = mpu_set_sample_rate(AI_INV_DEFAULT_MPU_HZ);    // 设置采样率
        if (ret)
            return -4;
        // 设置磁力计采样率
        ret = mpu_set_compass_sample_rate(100 / AI_INV_COMPASS_READ_MS);
        if (ret)
            return -5;
        ...
        inv_set_compass_sample_rate(AI_INV_COMPASS_READ_MS * 1000L);
        inv_set_gyro_orientation_and_scale(
            inv_orientation_matrix_to_scalar(ai_gyro_orien),
            (long)gyro_fsr << 15);
        inv_set_accel_orientation_and_scale(
            inv_orientation_matrix_to_scalar(ai_gyro_orien),
            (long)gyro_fsr << 15);
        inv_set_compass_orientation_and_scale(
            inv_orientation_matrix_to_scalar(ai_comp_orien),
            (long)compass_fsr << 15);
        ...
        ret = dmp_set_fifo_rate(AI_INV_DEFAULT_MPU_HZ);
        if (ret)
            return -9;
        ret = ai_inv_run_self_test();
        if (ret != 0)
            return -10;
...
/*
********************************************************************************
*    Function: ai_mpu_dmp_get_data
* Description: 得到dmp处理后的数据(注意,本函数需要比较多堆栈,局部变量有点多)
*       Input: pitch -> 俯仰角,精度:0.1°,范围:-90.0° <---> +90.0°
...
********************************************************************************
*/
int ai_mpu_dmp_get_data(float *pitch, float *roll, float *yaw)
{
    short gyro[3], accel[3], sensors;
    unsigned char more;
    long quat[4];
    unsigned long sensor_timestamp;
    float q0 = 1.0f, q1 = 0.0f, q2 = 0.0f, q3 = 0.0;
...
/*
********************************************************************************
*    Function: ai_mpu_mpl_get_data
* Description: 得到mpl处理后的数据(注意,本函数需要比较多堆栈,局部变量有点多)
*       Input: pitch -> 俯仰角,精度:0.1°,范围:-90.0° <---> +90.0°
*               roll -> 横滚角,精度:0.1°,范围:-180.0°<---> +180.0°
*                yaw -> 航向角,精度:0.1°,范围:-180.0°<---> +180.0°
*      Output: None
...
********************************************************************************
*/
int ai_mpu_mpl_get_data(float *pitch, float *roll, float *yaw)
{
    ...
    if (dmp_read_fifo(gyro, accel_short, quat,
